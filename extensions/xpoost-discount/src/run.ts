...
export function run(input: RunInput): FunctionRunResult {
  ...
  for (const line of lines) {
    ...
    if (rawVal.startsWith("fixed:")) {
      const fixedAmount = parseFloat(rawVal.replace("fixed:", ""));
      if (isNaN(fixedAmount) || fixedAmount <= 0) continue;
      ...
      discounts.push({
        ...
        message: "Volume Discount (-" + fixedAmount.toFixed(2) + " each)"
      });
    } else {
