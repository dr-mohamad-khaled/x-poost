    ...
    blocks.forEach(function (root) {
      ...
      loadConfig(proxyPath, root)
        .then(function (data) {
          ...
          // 1. Sync threshold from App Dashboard config (0 = Always Show for all stock levels)
          ...
          // 2. Sync minStock, maxStock, and stockSource if available
          if (cfg.minStock) minStock = parseInt(cfg.minStock, 10);
          if (cfg.maxStock) maxStock = parseInt(cfg.maxStock, 10);
          if (cfg.stockSource) stockSource = cfg.stockSource;

          // 3. Product Targeting Check
          ...
          // 4. Low stock threshold visibility check
          ...
          if (!isNaN(curQty)) {
            if (threshold === 0) {
              // 0 means ALWAYS SHOW regardless of how high stock is!
              root.classList.remove("xpp-ps-hidden");
            } else if (threshold > 0 && curQty > threshold) {
              root.classList.add("xpp-ps-hidden");
              return;
            } else {
              root.classList.remove("xpp-ps-hidden");
      ...
      function updateStockDisplay(newQuantity) {
        ...
        if (threshold > 0 && qty > threshold) {
          ...
          // threshold === 0 means ALWAYS SHOW for all inventory quantities
          root.classList.remove("xpp-ps-hidden");
        }
